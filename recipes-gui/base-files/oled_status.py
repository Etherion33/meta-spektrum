...
def detect_ip(preferred_iface: str = "wlan0") -> str:
...
def render_dashboard(
    ...
    *,
    ...
    if use_compact_ip:
        ...
        draw.text((16 + offset_x, main_y + offset_y), _shorten(ip_text, 18), font=small_font, fill=255)
    else:
        value_y = max(main_y - 1, main_y + (small_h - main_h) // 2)
        draw.text((2 + offset_x, main_y + 1 + offset_y), "IP", font=small_font, fill=255)
        draw.text((18 + offset_x, value_y + offset_y), _shorten(ip_text, 17), font=main_font, fill=255)
...
def main() -> int:
    ...
    while True:
        ...
        if now_monotonic >= next_ip_refresh_at:
            ...
            cached_ip = detect_ip(preferred_iface)
